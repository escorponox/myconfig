...
enum { QWERTY, NUMPAD, FUNCTION, CALT, CSHIFT, CSHIFTALT }; // layers
...
const Key keymaps[][ROWS][COLS] PROGMEM = {
  ...
  (Key_Escape,    Key_1, Key_2, Key_3, Key_4, Key_5, Key_LEDEffectNext,
   ...
   Key_Delete,    Key_Z, Key_X, Key_C, Key_V, Key_B, Key_Escape,
  ...
  (___,                     Key_F1,                         Key_F2,                       Key_F3,               Key_F4,               Key_F5,      XXX,
   ...
   Key_Home,                M(ARROW),                       M(ARROW_OBJ),                 M(ARROW_BLOCK),       M(EXPORT_CONST),      LGUI(Key_Z),
   Key_End,                 LGUI(LSHIFT(Key_Z)),            ___,                          ___,                  M(IMPORT_FROM),       ___,         ___,
   ShiftToLayer(CSHIFTALT), Key_Delete,                     ShiftToLayer(CALT),           ShiftToLayer(CSHIFT),
   ___,

   Consumer_ScanPreviousTrack, Key_F6,                 Key_F7,                   Key_F8,                   Key_F9,               Key_F10,               Key_F11,
   Consumer_PlaySlashPause,    Consumer_ScanNextTrack, Key_LeftCurlyBracket,     Key_RightCurlyBracket,    Key_LeftBracket,      Key_RightBracket,      Key_F12,
                               Key_LeftArrow,          Key_DownArrow,            Key_UpArrow,              Key_RightArrow,       ___,                   Key_Backslash,
...
const macro_t *exportConst() {
  return MACRO(T(E), T(X), T(P), T(O), T(R), T(T), T(Space), T(C), T(O), T(N), T(S), T(T), T(Space));
}

const macro_t *importFrom() {
  return MACRO(T(I), T(M), T(P), T(O), T(R), T(T), T(Space), T(LeftCurlyBracket), T(Space), T(Space), T(RightCurlyBracket), T(Space), T(F), T(R), T(O), T(M), T(Space), T(Quote), T(Quote), T(LeftArrow));
}
...
const macro_t *arrowObj() {
  return MACRO(D(LeftShift), T(9), U(LeftShift), T(LeftCurlyBracket), T(RightCurlyBracket), D(LeftShift), T(0), U(LeftShift), T(Space), T(Equals), D(LeftShift), T(Period), U(LeftShift), T(Space), D(LeftShift), T(9), U(LeftShift), T(LeftCurlyBracket), T(RightCurlyBracket), D(LeftShift), T(0), U(LeftShift), T(LeftArrow), T(LeftArrow));
}

const macro_t *arrowBlock() {
  return MACRO(D(LeftShift), T(9), U(LeftShift), T(LeftCurlyBracket), T(RightCurlyBracket), D(LeftShift), T(0), U(LeftShift), T(Space), T(Equals), D(LeftShift), T(Period), U(LeftShift), T(Space), T(LeftCurlyBracket), T(RightCurlyBracket), T(LeftArrow));
}
